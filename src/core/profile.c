...
struct ugl_profiler_t* ugl_profiler_create(int count, char* tag)
{
	struct ugl_profiler_t* prof = (struct ugl_profiler_t*) malloc(
		sizeof(struct ugl_profiler_header_t) + 
		count*sizeof(struct ugl_profiler_stamp_t));
	prof->h.pos=0;
	prof->h.tag=tag;
	prof->h.count=count;
...
void ugl_profiler_print(struct ugl_profiler_t* prof)
{
	...
	{
		delta = (float) (prof->data[i].stamp.tv_sec - prev.tv_sec) + 
			0.000001*((float)(prof->data[i].stamp.tv_usec - 
						prev.tv_usec));
		abs = (float) (prof->data[i].stamp.tv_sec - prof->h.start.tv_sec) + 
			0.000001*((float)(prof->data[i].stamp.tv_usec - 
						prof->h.start.tv_usec));
